...
class CatalogueOnlyConfig(OscarConfig):
    ...
    def ready(self):
        ...
        self.lading_view = get_class(
            "catalogue.views", "LandingPageView", "sierra_grande"
        )
    ...
    def get_urls(self):
        urls = super().get_urls()
        urls += [
            path("", self.lading_view.as_view(), name="index"),
            path("all-products/", self.catalogue_view.as_view(), name="browse"),
            re_path(
