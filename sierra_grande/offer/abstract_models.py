...
class AbstractRange(models.Model):
    ...
    def products_excluded(self):
        """
        Returns a queryset of products that are effectively excluded by this range's rules.
        This includes:
        1. Products explicitly listed in `excluded_products`.
        2. Products whose categories are in `excluded_categories`
           AND which are NOT otherwise included in the range.

        The logic identifies products that match any explicit exclusion criteria
        and then filters out any that are ultimately included by the range's
        overall logic (as determined by `self.product_queryset()`).
        """

        # 1. Identify all products that are explicitly excluded by product ID.
        Product = self.excluded_products.model
        explicitly_excluded_ids = self.excluded_products.values_list("id", flat=True)

        # 2. Build a Q object for products that fall into an excluded category.
        # This handles both parent products and child products through their parents.
        ...
        if self.excluded_categories.exists():
            ...
            # Products directly linked to excluded categories
            category_excluded_q |= Q(categories__in=expanded_excluded_categories)
            # Child products whose parents are linked to excluded categories
            category_excluded_q |= Q(
                parent__categories__in=expanded_excluded_categories
            )

        # 3. Combine all potential exclusion criteria (explicit products OR categories).
        # This queryset includes all products that *could* be excluded by the range.
        potentially_excluded_qs = Product.objects.filter(
            Q(id__in=explicitly_excluded_ids) | category_excluded_q
        ).distinct()

        # 4. Crucially, filter out any products from the `potentially_excluded_qs`
        # that are actually *included* in the range.
        # The `self.product_queryset()` method provides the definitive list
        # of products *actually* included after all Oscar's rules are applied.
        # If a product is in `potentially_excluded_qs` but also in `self.product_queryset()`,
        # it means an inclusion rule overrode its exclusion, so it's not effectively excluded.
        actually_included_ids = self.product_queryset.values_list("id", flat=True)

        # The final set of effectively excluded products
        effectively_excluded_qs = potentially_excluded_qs.exclude(
            id__in=actually_included_ids
        )

        return effectively_excluded_qs.order_by("title")  # Order for consistent display
